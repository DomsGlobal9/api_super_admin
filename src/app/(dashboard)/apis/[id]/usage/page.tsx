...
import { Activity, CheckCircle, Clock, Server, Key, Calendar, Filter, Search, Users } from 'lucide-react';
import { MetricCardSkeleton, TableSkeleton } from '@/components/ui/Skeletons';

export default function ApiUsagePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
    ...
    async function loadInitial(showLoader = true) {
      try {
        ...
        const [usageData, overviewData] = await Promise.all([
          apisClient.getUsage(id, filters),
          apisClient.getOverview(id)
        ]);
        if (isMounted) {
          setUsage(usageData);
          setClients(overviewData?.clients || []);
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  useEffect(() => {
    ...
    async function loadLogs(showLoader = true) {
      try {
        ...
        const logsData = await apisClient.getLogs(id, { pageSize: 10, ...filters });
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-950 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white">Live API Requests</h3>
        ...
        {logsLoading ? (
          ...
          <div className="p-8 text-center text-gray-500 text-sm">
            No API requests found for this microservice.
          </div>
        )}
      </div>
    </div>
